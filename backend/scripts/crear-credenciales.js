...
async function crearCredencialesPrueba() {
    try {
        ...
        // Hash de contraseñas
        const hashExterno = await bcrypt.hash('123456', 8);
        const hashAdmin = await bcrypt.hash('admin123', 8);
        const hashTesorero = await bcrypt.hash('tesorero123', 8);
        const hashSeminarista = await bcrypt.hash('semi123', 8);
