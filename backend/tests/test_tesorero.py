...
def test_tesorero_can_access_dashboard(logged_in_page: Page):
    ...
    # Verificar tarjetas de reportes específicas del tesorero
    expect(page.locator("text=/Reporte Mensual/i")).to_be_visible()
    expect(page.locator("text=/Ocupación Cabañas/i")).to_be_visible()
...
def test_tesorero_can_view_reportes(logged_in_page: Page):
    ...
    expect(page.get_by_role("heading", name="Reportes")).to_be_visible(timeout=15000)
    
    # Verificar elementos específicos de reportes
...
def test_tesorero_cannot_access_admin_sections(logged_in_page: Page):
    ...
    assert "/tesorero" in current_url or "/admin/certificaciones" not in current_url, \
           "El tesorero pudo acceder a sección exclusiva de admin"
