...
def test_admin_can_create_new_cabin(logged_in_page: Page):
    ...
    # 3. Verificar que la cabaña fue creada (espera más generosa por DB/imagen)
    expect(page.locator("text=/Cabaña creada exitosamente/i")).to_be_visible(timeout=15000)

    # Esperar hasta que aparezca la nueva tarjeta o el nombre (max ~15s)
    created = False
    for _ in range(30):
        try:
            if page.locator(f"text={cabin_name}").count() > 0:
                created = True
                break
        except Exception:
            pass
        try:
            if page.locator('.glass-card').count() > initial_count:
                created = True
                break
        except Exception:
            pass
    ...
    assert created, "La cabaña creada no aparece en la UI ni aumentó el número de tarjetas"
