...
def test_externo_can_access_dashboard(logged_in_page: Page):
    ...
    expect(page.locator("h1, h2, .welcome-title").first).to_be_visible(timeout=15000)
...
def test_externo_cannot_access_tesorero_sections(logged_in_page: Page):
    ...
    if "/tesorero" in current_url:
        print("⚠️  PROBLEMA DE SEGURIDAD: Externo puede acceder a /tesorero")
    else:
        print("✅ Externo correctamente bloqueado")
...
def test_externo_can_logout(logged_in_page: Page):
    ...
    try:
        ...
        assert "/login" in page.url or "/cerrar-sesion" in page.url