...
def test_admin_can_create_new_programa(logged_in_page: Page):
    ...
    expect(page.get_by_role("heading", name="Crear Nuevo Programa")).to_be_visible(timeout=10000)

    # Rellenar el formulario con los labels correctos del modal
    timestamp = int(time.time())
    program_name = f"Programa de Prueba {timestamp}"
    
    # Campos básicos
    page.get_by_label("Título del Programa *").fill(program_name)
    page.get_by_label("Descripción del Programa *").fill("Descripción del programa de prueba.")
    
    # Selects - usar name en lugar de label
    page.locator("select[name='tipo']").select_option("curso")
    page.locator("select[name='modalidad']").select_option("presencial")
    
    # Campos numéricos y de texto
    page.get_by_label("Duración *").fill("8 Semanas")
    page.get_by_label("Precio *").fill("150000")
    page.get_by_label("Cupos Disponibles *").fill("25")
    
    # Fechas
    today = datetime.now()
    start_date = (today + timedelta(days=10)).strftime("%Y-%m-%d")
    end_date = (today + timedelta(days=70)).strftime("%Y-%m-%d")
    page.get_by_label("Fecha de Inicio *").fill(start_date)
    page.get_by_label("Fecha de Fin *").fill(end_date)
    
    # Profesor
    page.get_by_label("Profesor/Instructor *").fill("Profesor de Prueba")

    # Guardar programa
    save_button = page.locator("button:has-text('Crear Programa')")
    save_button.click()

    # Esperar a que el modal se cierre
    page.wait_for_timeout(3000)
    
    # Verificar que volvemos a la página de programas y hay contenido
    page.wait_for_selector("table, .program-card, .glass-card", timeout=10000)
    
    # Verificar que hay al menos un programa en la lista
    # (puede ser el que acabamos de crear u otros existentes)
    expect(page.locator("table tbody tr, .program-card, .glass-card").first).to_be_visible(timeout=5000)
...
def test_tesorero_can_view_programas(logged_in_page: Page):
    ...
    # Verificar acceso
    expect(page.get_by_role("heading", name="Gestión Programas")).to_be_visible(timeout=15000)
