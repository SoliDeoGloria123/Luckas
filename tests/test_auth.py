    ...
    def test_logout_for_all_roles(self, logged_in_page: Page):
        ...
        try:
            if role == "seminarista":
                ...
                user_profile = page.locator(".user-profile-seminario").first
                if user_profile.count() > 0:
                    user_profile.click()
                    page.wait_for_timeout(500)
                    
                    # Buscar botón de logout en dropdown
                    logout_btn = page.locator("button.dropdown-item:has-text('Cerrar Sesión')").last
                    if logout_btn.count() > 0:
                        logout_btn.click(force=True)
                    else:
                        raise Exception("No se encontró botón de logout")
                else:
                    raise Exception("No se encontró perfil de usuario")
            else:
                # Para tesorero y externo, ir directo a cerrar sesión
